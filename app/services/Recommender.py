...
class Recommender:
    ...
    def popularity_based_recommendation(self, df, rating_cols):
        ...
        topn_candidates = sorted_df[sorted_df['total_rating']
                                    == max_total_rating]
        ...
        if len(topn_candidates) <= self.recommend_num:
            ...
            sampled_rows = random.sample(
                topn_candidates.index.tolist(), self.recommend_num)
    ...
    def matrix_factorization_recommendation(self, user_id, df):
        if 'user_id' not in df.columns or 'item_id' not in df.columns:
            raise ValueError(
                "DataFrame must contain 'user_id' and 'item_id' columns")

        item_ids = df['item_id'].unique()
        predictions = defaultdict(float)

        for item_id in item_ids:
            predictions[item_id] = self.matrix_factorization_model.predict(
                user_id, item_id, verbose=True).est
        ...
        self.matrix_factorization_recommendation_result = [
            item[0] for item in recommended_items]
        pass
    ...
    def hybrid_recommendation(self):
        ...
        content_based_df = self.weighted_content_based_recommend_result[[
            'HouseID', 'weighted_similarity']]
        scaler = MinMaxScaler(feature_range=(-1, 1))
        matrix_factorization_df['est'] = scaler.fit_transform(
            matrix_factorization_df[['est']])
        merged_df = pd.merge(matrix_factorization_df,
                             content_based_df, on='HouseID')
        merged_df['weighted_score'] = self.alpha * \
            merged_df['weighted_similarity'] + \
            (1 - self.alpha) * merged_df['est']
        self.hybrid_recommendations_result = merged_df.sort_values(
    ...
    def noRating_recommend(self):
        ...
        self.weighted_content_based_recommendation(
            self.data, self.user_preference, self.weight_vector)
    ...
    def get_result(self):
        if self.mode == "content_based":
            print("no rating")
            result_w = self.weighted_content_based_recommend_result.head(
                self.recommend_num)
            ...
            print("rating")
            result = self.hybrid_recommendations_result.head(
                self.recommend_num)
            result_list = result['HouseID'].tolist()
            return result_list
    ...
    def model_update(self, df):
        ...
        # assumes datafile contains: user, item, ratings (in this order)
