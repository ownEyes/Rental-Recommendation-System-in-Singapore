...
def submit_data():
    ...
    if form.validate_on_submit():
        ...
        user_input_data['public_facilities'] = ', '.join(
            form.public_facilities.data)
        user_input_data['cooking_facilities'] = ', '.join(
            form.cooking_facilities.data)
        user_input_data['interior_facilities'] = ', '.join(
            form.interior_facilities.data)
        ...
        amv = amenities_to_vector([form.public_facilities.data, form.cooking_facilities.data,
                                  form.interior_facilities.data, form.other_needs.data])
        geocoding = get_geocoding(
            user_input_data['region'], user_input_data['postcode'])
...
def priadv():
    ...
    predadv = {
        'price': 'S$ '+str(int(user_input_data.get('price', '')))+' /month',
...
def mapgenerate():
    ...
    userin = ['The information of your own residents', str(user_input_data.get('numbedrooms', '')+' bedroom · '+user_input_data.get('numbeds', '')+' bed · ' + user_input_data.get('numbathrooms', '') + ' '+user_input_data.get('bathroomtype', '') + ' bath'), user_input_data.get('region', ''), str(user_input_data.get('accommodates', '') + ' accommodate'), user_input_data.get('roomtype', ''),
              ...
              str(user_input_data.get('stop_id', ''))+' ' +
              str(user_input_data.get('mrt_name', '')),
              str(int(user_input_data.get('mall_dis', '')*1000))+'m to '+str(user_input_data.get('mall_name', '')) +
              '('+str(user_input_data.get('mall_adress', '')) +
              ')', user_input_data.get('latitude', 1.352083),
